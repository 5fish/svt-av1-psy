...
static void tx_type_search(PictureControlSet *pcs, ModeDecisionContext *ctx, ModeDecisionCandidateBuffer *cand_bf,
    ...
    for (int tx_type_group_idx = 0; tx_type_group_idx < tx_type_tot_group; ++tx_type_group_idx) {
        ...
        for (int tx_type_idx = 0; tx_type_idx < TX_TYPES; ++tx_type_idx) {
            ...
            if (pcs->scs->static_config.variance_md_bias && cand_bf->variance_md_mode_bias >= 1) {
                // DCT is blurry but creates less artefacts it looks like
                // FLIPADST seems to be prone to artefacts while not being particularly good.
                switch (tx_type) {
                    case DCT_DCT:
                        txb_full_distortion_txt[DIST_SSD][tx_type][DIST_CALC_RESIDUAL] *= 1.36; break;
                    case FLIPADST_FLIPADST: case DCT_FLIPADST: case FLIPADST_DCT:
                        txb_full_distortion_txt[DIST_SSD][tx_type][DIST_CALC_RESIDUAL] *= 1.18; break;
                    case ADST_DCT: case DCT_ADST: case ADST_FLIPADST: case FLIPADST_ADST:
                        txb_full_distortion_txt[DIST_SSD][tx_type][DIST_CALC_RESIDUAL] *= 1.05; break;
                    case V_FLIPADST: case H_FLIPADST:
                        txb_full_distortion_txt[DIST_SSD][tx_type][DIST_CALC_RESIDUAL] *= 1.03; break;
                    // case V_DCT: case H_DCT:
                    //     txb_full_distortion_txt[DIST_SSD][tx_type][DIST_CALC_RESIDUAL] *= 1.00; break;
                    case ADST_ADST:
                        txb_full_distortion_txt[DIST_SSD][tx_type][DIST_CALC_RESIDUAL] *= 0.97; break;
                    case V_ADST: case H_ADST:
                        txb_full_distortion_txt[DIST_SSD][tx_type][DIST_CALC_RESIDUAL] *= 0.95; break;
                    case IDTX:
                        txb_full_distortion_txt[DIST_SSD][tx_type][DIST_CALC_RESIDUAL] *= 0.92; break;
                    default: break;
                }
            }
...
static void variance_md_bias_core(PictureControlSet *pcs, ModeDecisionContext *ctx, struct ModeDecisionCandidateBuffer *cand_bf) {
    ...
    if (variance >= main_thr && variance >= 16)
        cand_bf->variance_md_mode_bias = 2;
    else if (variance >= main_thr >> 1 && variance >= 12)
